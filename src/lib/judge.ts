 ...
 */
const lowerIfAlpha = (ch: string) => (/[a-z]/i.test(ch) ? ch.toLowerCase() : ch);
 ...
 */
export function judgeChar(answerEn: string, cursor: number, key: string): JudgeResult {
	const exp = answerEn[cursor] ?? "";
	if (key === "\n" || key === "\b") return { ok: false, expected: exp, received: key };
