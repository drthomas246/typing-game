 ...
 * @param {ReactNode} props.children - プロバイダーがラップする子要素。
 *
 * @returns {JSX.Element} グローバルなコンテキストを提供するJSX要素。
 */
export default function PageProvider({ children }: { children: ReactNode }) {
	...
	const battle = useMemo(() => (snap?.state?.settings?.mode ?? "battle") === "battle", [snap]);
	const sort = useMemo(() => (snap?.state?.settings?.sort ?? "random") === "reverse", [snap]);
