...
import type { Action, EngineOptions, EngineState, SoundCtl } from "@/types/index";
...
export function useBattle(
	...
	const damagePerSentence = Math.max(1, opts.damagePerSentence ?? opts.damagePerHit ?? 10);
	...
	const onMiss = useCallback(
		(s: EngineState) => {
			...
			const justDied = s.playerHp > 0 && Math.max(0, s.playerHp - damagePerMiss) === 0;
	...
	const onSentenceClear = useCallback(
		(s: EngineState) => {
			...
			const killedNow = s.enemyHp > 0 && Math.max(0, s.enemyHp - damagePerSentence) === 0;
			dispatch({
				type: "DAMAGE_ENEMY",
