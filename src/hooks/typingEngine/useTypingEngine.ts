...
import { addMissedProblem } from "@/db";
...
export function useTypingEngine(
  ...
  const onMissWithRecord = useCallback(
    (payload: EngineState) => {
      // まず既存のミス処理
      onMiss(payload);
      ...
      if (!opts.learningMode) {
        ...
        const pair = idx >= 0 ? getPair(idx) : undefined;
        if (pair) void addMissedProblem(pair);
      }
    },
    [onMiss, opts.learningMode, state.index, getPair],
  );
